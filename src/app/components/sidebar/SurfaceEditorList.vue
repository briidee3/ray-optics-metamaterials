...
<template>
  <div class="surface-editor-item-list" role="list" ref="listRoot" @dragover.prevent="onDragOverList" @drop.prevent="onDropList" @click.stop>
    <div class="surface-editor-item-list-rows">
      <div
        v-for="(item, index) in items"
        :key="itemKeyValue(item, index)"
      class="surface-editor-item-list-row"
      ...
      >
        <div class="surface-editor-item-list-left">
        <label class="surface-editor-item-checkbox" :class="{ 'is-checked': isSelected(item, index) }" @click.stop>
            <input
              type="checkbox"
              class="surface-editor-item-checkbox-input"
              :checked="isSelected(item, index)"
              @change="onToggleSelect($event, item, index)"
            >
            <span class="surface-editor-item-checkbox-mark" aria-hidden="true"></span>
          </label>
          <button
            type="button"
            class="surface-editor-item-drag surface-editor-item-control"
            draggable="true"
        ...
        <div class="surface-editor-item-list-content">
          <slot name="content" :item="item" :index="index" />
        </div>

        <div class="surface-editor-item-list-actions">
          <button
            type="button"
            class="surface-editor-item-action surface-editor-item-control"
          ...
          <button
            type="button"
            class="surface-editor-item-action surface-editor-item-control"
            aria-label="Use transformation optics"
            @click="$emit('enable-to', item, index)"
          @click.stop
          >
            <!-- <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" fill="currentColor" stroke="currentColor" viewbox="0 0 16 16">
            ...
            <svg fill="currentColor" height="11" width="11" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" xml:space="preserve">
              <path d="M162.909,465.455h-23.273c-12.87,0-23.273,10.426-23.273,23.273S126.767,512,139.636,512h23.273 c12.87,0,23.273-10.426,23.273-23.273S175.779,465.455,162.909,465.455z"/> <path d="M279.273,465.455H256c-12.87,0-23.273,10.426-23.273,23.273S243.13,512,256,512h23.273 c12.87,0,23.273-10.426,23.273-23.273S292.143,465.455,279.273,465.455z"/> <path d="M69.818,139.636H46.545c-12.87,0-23.273,10.426-23.273,23.273c0,12.846,10.403,23.273,23.273,23.273h23.273 c12.87,0,23.273-10.426,23.273-23.273C93.091,150.063,82.688,139.636,69.818,139.636z"/> <path d="M23.273,395.636c12.87,0,23.273-10.426,23.273-23.273v-23.273c0-12.847-10.403-23.273-23.273-23.273S0,336.244,0,349.091 v23.273C0,385.21,10.403,395.636,23.273,395.636z"/> <path d="M23.273,279.273c12.87,0,23.273-10.426,23.273-23.273v-23.273c0-12.847-10.403-23.273-23.273-23.273S0,219.881,0,232.727 V256C0,268.847,10.403,279.273,23.273,279.273z"/> <path d="M349.091,418.909c-12.87,0-23.273,10.426-23.273,23.273v23.273c0,12.847,10.403,23.273,23.273,23.273 c12.87,0,23.273-10.426,23.273-23.273v-23.273C372.364,429.335,361.961,418.909,349.091,418.909z"/> <path d="M46.545,465.455c0-12.846-10.403-23.273-23.273-23.273S0,452.608,0,465.455v23.273C0,501.574,10.403,512,23.273,512 h23.273c12.87,0,23.273-10.426,23.273-23.273S59.415,465.455,46.545,465.455z"/> <path d="M488.355,0H209.827c-13.056,0-23.645,10.589-23.645,23.645v115.991h-23.273c-12.87,0-23.273,10.426-23.273,23.273 c0,12.846,10.403,23.273,23.273,23.273h23.273v115.991c0,13.056,10.589,23.645,23.645,23.645h115.991v23.273 c0,12.846,10.403,23.273,23.273,23.273c12.87,0,23.273-10.426,23.273-23.273v-23.273h115.991 c13.056,0,23.645-10.589,23.645-23.645V23.645C512,10.589,501.411,0,488.355,0z"/>
            </svg>
          </button>
        </div>
      </div>
    ...
    <button v-if="showAddButton" type="button" class="surface-editor-item-list-add" @click="$emit('create')">
...
export default {
  ...
  emits: ['update:selectedIds', 'remove', 'duplicate', 'reorder', 'create', 'selection-change', 'hover', 'select', 'enable-to'],
  data() {
    return {
      dragIndex: null,
      hoverIndex: null,
  ...
  methods: {
    ...
    indexFromEventTarget(target) {
      const row = target?.closest ? target.closest('.surface-editor-item-list-row') : null
      if (!row || !row.dataset) {
        return null
      }
...
<style scoped>
.surface-editor-item-list {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.surface-editor-item-list-rows {
  display: flex;
  flex-direction: column;
  gap: 8px;
  position: relative;
...
.surface-editor-item-list-row.is-dragging {
...
.surface-editor-item-list-row.is-drop-target {
  background: rgba(255, 255, 255, 0.1);
}

.surface-editor-item-list-row.is-selected {
  background: rgba(86, 219, 240, 0.25);
}

.surface-editor-item-list-row.is-drop-before::before,
.surface-editor-item-list-row.is-drop-after::after {
  content: '';
  position: absolute;
  left: 8px;
  right: 8px;
...
.surface-editor-item-checkbox-input {
  position: absolute;
  opacity: 0;
  pointer-events: none;
}

.surface-editor-item-checkbox-mark {
  width: 14px;
  height: 14px;
...
.surface-editor-item-checkbox-mark::after {
  content: '';
  position: absolute;
  left: 3px;
  top: 1px;
...
.surface-editor-item-checkbox.is-checked {
  opacity: 1;
  pointer-events: auto;
}

.surface-editor-item-checkbox.is-checked .surface-editor-item-checkbox-mark {
  background: rgba(86, 219, 240, 0.35);
  border-color: rgba(86, 219, 240, 0.75);
  box-shadow: 0 0 0 1px rgba(86, 219, 240, 0.25);
}

.surface-editor-item-checkbox.is-checked .surface-editor-item-checkbox-mark::after {
  opacity: 1;
}

.surface-editor-item-drag {
  border: none;
  padding: 1px;
...
.surface-editor-item-list-content {
  flex: 1;
  min-width: 0;
  display: flex;
  align-items: center;
...
.surface-editor-item-list-actions {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 4px;
...
.surface-editor-item-list-row:hover .surface-editor-item-control,
.surface-editor-item-list-row:hover .surface-editor-item-checkbox {
  opacity: 1;
  pointer-events: auto;
}
</style>
