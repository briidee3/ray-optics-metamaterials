...
class CurveGrinGlass extends CurveObjMixin(BaseGrinGlass) {
    ...
    draw(canvasRenderer, isAboveLight, isHovered) {
        ...
        if (this.notDone) {
            ...
            // Set up paths for drawing and draw them
            for (var i = 0; i < this.path.length - 2; i ++) {
                // Set up path for drawing
                //ctx.bezierCurveTo(this.controlPoints[i].x, this.controlPoints[i].y, this.controlPoints[(i + 1)].x, this.controlPoints[(i + 1)].y, this.path[(i + 1)].x, this.path[(i + 1)].y);
                //ctx.moveTo(this.path[(i + 1)].x, this.path[(i + 1)].y);                         // Go to the next starting point
                this.path[i]
            }
            // Draw paths
            ctx.globalAlpha = 1;
            ...
            // Draw boxes around control points and lines connecting them to corresponding path points.
            ctx.beginPath();
            ctx.moveTo(this.path[0].x, this.path[0].y);

            for (var i = 0; i < this.path.length; i++) {
                ctx.rect(this.controlPoints[i].x - 1.5 * ls, this.controlPoints[i].y - 1.5 * ls, 3 * ls, 3 * ls);
                ctx.rect(this.controlPoints[i + 1].x - 1.5 * ls, this.controlPoints[i + 1].y - 1.5 * ls, 3 * ls, 3 * ls);
                ctx.moveTo(this.controlPoints[i].x, this.controlPoints[i].y)                    // Move to control point
                ctx.lineTo(this.path[i].x, this.path[i].y)                                      // Make line connecting box and path point
            }
            // Draw paths
            ctx.globalAlpha = 1;
            ctx.strokeStyle = 'rgb(250,20,250)';
            ctx.lineWidth = 1 * ls;
            ctx.stroke();
    ...
    move(dx, dy) {
        ...
        console.log("CP LENGTH: ", this.controlPoints.length);
        this.origin.x += dx;
        this.origin.y += dy;
        for (var i = 0; i < this.path.length; i++) {
            this.path[i].x += dx;
    ...
    onConstructMouseDown(mouse, ctrl, shift) {
        ...
        // If there's not the expected number of control points for the current number of path points, set current point to be new control point
        else if (this.path.length > 1 && this.controlPoints.length < (this.path.length - 1) * 2) {
            // Add control points at current mouse point
            this.controlPoints.push({ x: mousePos.x, y: mousePos.y });
        } 
        // Otherwise, add new path point
        else {
         ...
         */ 
        if (this.path.length > 3 && mouse.snapsOnPoint(this.path[0]) && this.controlPoints.length < (this.path.length - 1) * 2) {
    ...
    drawGlass(canvasRenderer, isAboveLight, isHovered) {
        ...
        for (var i = 0; i < this.path.length - 1; i ++) {
            // Set up path for drawing
            this.draw(this.curve);//ctx.bezierCurveTo(this.controlPoints[i].x, this.controlPoints[i].y, this.controlPoints[(i + 1)].x, this.controlPoints[(i + 1)].y, this.path[(i + 1)].x, this.path[(i + 1)].y);
            //ctx.moveTo(this.path[(i + 1)].x, this.path[(i + 1)].y);                         // Go to the next starting point
        }
        this.fillGlass(canvasRenderer, isAboveLight, isHovered);
    }
